  ...
  return (
    <div className="login-wrap">
      <div className="login-card">
        <div className="login-logo-chip"><img src="/logo-ace.png" alt="ACE" /></div>
        <h1 className="login-title">Order To <span>Delivery</span></h1>
        <p className="login-tag">Acemark Stationers · Order se Payment tak ka pura follow-up</p>
        <button className="btn-google" onClick={signIn} disabled={busy}>
          ...
          {busy ? 'Google par le ja rahe hain…' : 'Sign in with Google'}
        </button>
        {err && <p className="err">{err}</p>}
        <p className="login-foot">Sirf authorised team ke liye · <b>www.acemark.in</b></p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="app">
      <header className="topbar">
        <div className="brand">📋 <b>Order To Delivery</b> <span className="muted small">Acemark</span></div>
        <nav>
          {navTabs.map(([k, l]) => (
